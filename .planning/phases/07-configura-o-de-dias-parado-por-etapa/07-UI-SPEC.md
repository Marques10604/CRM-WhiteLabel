...
- Valores padrão no primeiro carregamento: **Contatado = 5 é obrigatório e travado** (D-04, paridade de comportamento com o hardcode atual). Os padrões de Novo e Negociação (campos novos, sem hardcode prévio equivalente) ficam a critério técnico do planner — sugestão de UI: mesmo valor `5` para os três, por consistência visual do formulário no primeiro acesso, mas isso não é uma decisão de UX travada pelo usuário.
