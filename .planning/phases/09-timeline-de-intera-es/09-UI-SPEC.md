...
| md | 16px | `gap-4` entre o composer de nota nova e a lista de entradas; padding interno do `DialogContent` (herdado do primitivo, `p-4`) |
...
| Destructive confirmation — botões | "Cancelar" (`variant="outline"`) / "Excluir" (`variant="destructive"`) — mesmo padrão de `DeleteLeadDialog` |
...
- **Lista de entradas (abaixo do composer, ordem cronológica decrescente):** cada entrada é uma linha (`flex flex-col gap-1`, `border-b border-zinc-200 py-3 last:border-b-0`, mesmo padrão de separador de `lead-table.tsx`), estruturada assim:
...
- **Estado vazio:** quando não há nenhuma entrada (nem WhatsApp nem nota), a lista é substituída por um bloco centralizado (`flex flex-col items-center gap-2 py-10 text-center`, mesmo espírito do empty state de `lead-table.tsx`) com heading (`text-[16px] font-semibold`) e body (`text-[14px] text-muted-foreground`) — o composer continua visível acima, o admin pode criar a primeira nota sem sair do dialog.
