...
last_activity: 2026-09-12 - Verificação humana da 260912-pzc concluída; corrigido bug real (SqliteError no ESCAPE do LIKE de buscarGlobal, commit 8099ad8)
progress:
  total_phases: 25
  completed_phases: 25
  total_plans: 81
...
- **23-06** ✅ COMPLETO e FECHADO (2 tasks + checkpoint de 5 rodadas reais, SUMMARY). Dataset de referência (`6120471`) + eval on-demand (`6401ca7`). Rodada 1 (`1f4704f`) e rodada 2 (`fb739ba`+`e49d6da`) NÃO discriminaram os 3 gold. Investigação de causa raiz (aprovada pelo usuário) achou dicotomia falsa no schema de achado (2 categorias, sem lugar pra relato anedótico isolado real) — corrigida em `f621c51` (3ª categoria `relato_qualitativo` + campo `evidencia` em `gatilhos_dor` + novo `.refine`, D-23-06), com fixtures/harness atualizados (`4845d6d`) e eval sincronizado (`6c19275`). Rodada 3 (`1e81375`): **os 3 vereditos gold vieram discriminados entre si pela 1ª vez** (aprofundar/mudar_angulo/abandonar), 1/3 batendo exatamente com o gabarito humano de 2026-09-04 — aceito como baseline pelo usuário. Code review da fase (`4551cf6`) achou CR-01/CR-02, corrigidos em `b233f80`. Investigação reaberta do cross-check de citações: fix de normalização/prompt aplicado (`1105f56`); rodadas 4 (`083a4b8`) e 5 (`c2e6cc1`, mesmo código, sem mudança) mediram variância — **nenhum dos 3 vereditos bateu entre as 2 rodadas idênticas**, e a rodada 5 parou no caso 3 por crédito de API zerado. Custo real total do dia: ~US$3,94 (spike 23-04 + 5 rodadas do 23-06).

### Dívida técnica FINAL da Fase 23 (não-bloqueante, fechada com achados documentados — não vai ser corrigida nesta sessão)

1. **Variância de veredito entre execuções idênticas — ACEITA como comportamento ESPERADO do sistema, não um bug.** Rodadas 4 e 5 do eval (mesmo código, mesma config, zero mudança entre elas) produziram vereditos diferentes nos 3 casos gold. Causa: busca adaptativa (cada execução descobre fatos de mercado diferentes) + LLM não-determinístico (mesmo com `effort:"low"`; `temperature` nem é honrado pelo provider). **Implicação de produto, permanente:** o veredito da IA é sempre uma sugestão inicial de uma única execução — a UI/copy do produto deve deixar isso claro, e o operador SEMPRE valida antes de agir (a Fase 24, que registra o veredito final do operador podendo divergir do sugerido, já foi desenhada em cima dessa premissa).
2. **Sufixo espúrio de 1-3 caracteres em citações de URL do cross-check — bug real, NÃO resolvido.** Instrumentação nova (commit `1105f56`) revelou que a maioria dos mismatches do cross-check FM2 não é "URL errada"/alucinação de conteúdo — é a URL REAL e exata (confirmável em "Fontes coletadas" do relatório) com um sufixo espúrio grudado no final (`','`, `-`, `1`, `2`, `/https`; ex.: `.../tabela-2026/','` quando a fonte real é `.../tabela-2026/`). Causa provável: artefato de serialização da saída estruturada (possível bleed-through de sintaxe de array/citação do provider) — **não confirmada**. Fix de normalização de URL (`normalizarUrl()`) e instrução de cópia literal no prompt já aplicados, mas não resolvem esse padrão específico (o sufixo faz a URL normalizada ainda diferir da real). **Pendente de investigação futura** (ex.: checar "URL citada menos N caracteres finais" contra fontes reais, ou capturar o texto bruto pré-validação Zod) — não investigado mais a fundo por falta de crédito de API nesta sessão (a rodada 5 parou exatamente por isso).
3. Chamada do LLM-judge falhando ocasionalmente (`NoObjectGeneratedError` na própria chamada do juiz, independente da geração) — o fix do commit `e49d6da` já garante que isso NÃO derruba mais o resultado da geração; dívida menor, não re-investigada.
4. Nenhum dos 3 itens acima bloqueia o início da Fase 24 — são características/limitações conhecidas do sistema de IA, não do produto (`src/` além do já corrigido).

### Pendente (fora do escopo deste executor de plano — orquestração de nível de fase)

- Regression gate, schema-drift gate, `gsd-verifier` — ainda não rodados para a Fase 23 inteira (`gsd-code-review` já rodou, `23-REVIEW.md`).
- **UAT humano** da fase: fluxo do botão em `/campanhas/[id]` (vazio → gerar → pending → resultado → regenerar → histórico), claro + escuro. `gerarDiagnostico()` já validado ponta a ponta contra a API real (spike do 23-04 + 5 rodadas do eval do 23-06) — só falta a passada visual humana.
...
- ~~**Shim global `gsd-sdk` quebrado (2026-09-09):**~~ **RESOLVIDO 2026-09-10.** A pasta "Todas as Pastas\Framework GDS" tinha sido movida/renomeada para `C:\Users\Vencedor\Desktop\Framework GSD\Framework GDS\` (nota: pasta pai "Framework GSD", subpasta "Framework GDS" com o typo antigo). Os 3 shims (`gsd-sdk`, `.cmd`, `.ps1` em `~/AppData/Roaming/npm/`) foram reapontados à mão para `C:\Users\Vencedor\Desktop\Framework GSD\Framework GDS\bin\gsd-sdk.js` e testados (`gsd-sdk query ...` volta a funcionar). Durante a janela quebrada, STATE.md + ROADMAP.md da Fase 23 foram anotados à mão (mantidos — a re-execução do `state.planned-phase` gerou churn de reformatação e foi revertida).
...
| 260912-pzc | Busca global (Ctrl/Cmd+K) com `cmdk` — leads/campanhas/nichos, gatilho na sidebar, deep-link `/leads?busca=`. Verificação humana achou e corrigiu bug real (`SqliteError` no `ESCAPE` do `LIKE`) | 2026-09-12 | 2721959, bc58cde, fde6043, 8099ad8 | [260912-pzc-adicionar-busca-global-ctrl-k-no-topo-do](./quick/260912-pzc-adicionar-busca-global-ctrl-k-no-topo-do/) |
...
| 260910-qnb | Sidebar do CRM: (1) `<body>` min-h-dvh + `<aside>` sticky top-0 h-dvh + overflow-y-auto — o painel do menu agora vai até o fim da janela sempre (feedback do usuário: "sidebar não chega até o final", com screenshot). (2) Marca "SOLO" com presença: logo 40px (size-10), nome 20px bold na cor do texto (text-sidebar-foreground) em vez de cinza 11px no canto. Gates verdes. | 2026-09-10 | 29d75ca | [260910-qnb-sidebar-do-crm-esticar-ate-a-altura-tota](./quick/260910-qnb-sidebar-do-crm-esticar-ate-a-altura-tota/) |
