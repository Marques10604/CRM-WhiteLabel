...
| 260828-gna | Corrigir deadlock ao arrastar card para "Perdido" no /pipeline (bug do UAT da Fase 11, Teste 5): setMotivoPerdaState({open:true}) saiu de dentro de startTransition(async → await new Promise); agora o drop para Perdido só enfileira + abre o modal (urgente), e "Salvar motivo" dispara uma nova transição normal que move+persiste. "Cancelar" só descarta (card nunca moveu). tsc limpo, bundle confirmado; drag ao vivo pendente de 1 confirmação manual (captura de tela da extensão quebrou na sessão). | 2026-08-28 | fbf7abd | [260828-gna-corrigir-deadlock-drag-perdido](./quick/260828-gna-corrigir-deadlock-drag-perdido/) |
