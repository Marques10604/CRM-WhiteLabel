...
**Exceção — fix pontual construído fora do plano nesta mesma sessão (`cbfb1bc`):** durante um teste real de importação, sub-nicho obrigatório travava o botão "Confirmar importação" (erro genérico "Não foi possível importar os leads") sempre que o CSV não tinha coluna de sub-nicho — o que é o caso real dele (planilha vem do cowork via scraping de Instagram, sem essa info). Implementado direto (sem passar pelo pipeline gsd-planner/gsd-executor, decisão consciente pra economizar tokens num fix pequeno e já bem entendido): linha sem sub-nicho agora importa com fallback `"A categorizar"` em vez de bloquear — `src/components/csv-import-preview-table.tsx`, `tsc`/`build` limpos. O combobox inline pra escolher o sub-nicho certo na hora continua disponível, só deixou de ser obrigatório. Isso é diferente e não substitui o Gap 5 (tela de configuração) nem a Task 1 pendente (botão de remoção de sub-nicho) — só resolve o bloqueio de import.
