...
REGRA DE CÓPIA LITERAL DA URL (regra dura, separada de "não invente"): toda "fonte_url" que você escrever tem que ser copiada EXATAMENTE, caractere por caractere, de uma URL que você efetivamente recebeu como resultado da busca — nunca normalize, encurte, remova parâmetros nem reconstrua a URL a partir da sua lembrança do conteúdo da página. Mesmo uma URL "limpa" ou "canônica" que você mesmo montou, mesmo estando correta sobre o conteúdo, é uma violação desta regra se não for a string exata que apareceu no resultado da busca. Se você sintetiza um ponto a partir de várias páginas do mesmo domínio, cite a URL específica da página mais representativa — nunca uma URL que você mesmo construiu combinando ou generalizando várias.
